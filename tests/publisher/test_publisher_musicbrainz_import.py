...
class TestResolveOrCreatePublisher:
    def test_creates_new_publisher_when_no_match(self, controller):
    ...
    def test_matches_by_mbid_without_creating_duplicate(self, controller):
        existing = controller.add.add_entity(
            "Publisher", publisher_name="Some Other Name", MBID=_label().mbid
        )
    ...
    def test_matches_by_name_and_backfills_mbid(self, controller):
        existing = controller.add.add_entity(
    ...
    def test_matches_by_alias(self, controller):
        canonical = controller.add.add_entity(
...
class TestResolveOrCreateFounderArtist:
    def test_creates_new_artist_when_no_match(self, controller):
    ...
    def test_matches_existing_artist_by_alias_and_backfills_mbid(self, controller):
        canonical = controller.add.add_entity("Artist", artist_name="A. Ertegun", MBID=None)
        controller.add.add_entity(
...
class TestApplyPublisherFounders:
    def test_skips_founder_already_linked(self, controller):
        publisher = controller.add.add_entity("Publisher", publisher_name="Atlantic Records")
        artist = controller.add.add_entity("Artist", artist_name="Ahmet Ertegun")
        controller.add.add_entity(
        ...
        apply_publisher_founders(controller, publisher, [founder])

        links = controller.get.get_all_entities(
            "PublisherFounder", publisher_id=publisher.publisher_id
        )
        assert len(links) == 1

    def test_adds_new_founder(self, controller):
        publisher = controller.add.add_entity("Publisher", publisher_name="Atlantic Records")
        founder = MBFounderRelation(mbid="22222222-2222-2222-2222-222222222222", name="Ahmet Ertegun")
...
class TestApplyPublisherHeadquarters:
    def test_skips_when_headquarters_already_set(self, controller):
        publisher = controller.add.add_entity("Publisher", publisher_name="Atlantic Records")
        hq_type = controller.add.add_entity("PlaceAssociationType", type_name="Headquarters")
        place = controller.add.add_entity("Place", place_name="New York City")
        controller.add.add_entity(
            "PlaceAssociation",
        ...
        apply_publisher_headquarters(controller, publisher, chain, {})

        assocs = controller.get.get_all_entities(
            "PlaceAssociation", entity_type="Publisher", entity_id=publisher.publisher_id
        )
        assert len(assocs) == 1
        assert assocs[0].place.place_name == "New York City"

    def test_creates_headquarters_when_none_exists(self, controller):
        publisher = controller.add.add_entity("Publisher", publisher_name="Atlantic Records")
        chain = [
...
class TestImportAlbumLabels:
    def test_creates_publisher_and_links_album(self, controller):
        album = controller.add.add_entity("Album", album_name="Genesis")

        failures = import_album_labels(controller, album, [_label()], {})

        assert failures == []
        assert len(album.publishers) == 1
        assert album.publishers[0].publisher_name == "Atlantic Records"

    def test_does_not_duplicate_link_for_already_linked_publisher(self, controller):
        album = controller.add.add_entity("Album", album_name="Genesis")
        publisher = controller.add.add_entity(
            "Publisher", publisher_name="Atlantic Records", MBID=_label().mbid
        )
        controller.add.add_entity(
