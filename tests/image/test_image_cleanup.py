...
unlinks or renames files under images/artist_images/ when their owning row
goes away.
"""
...
from src.image.image_cleanup import delete_managed_image, prune_orphaned_images, rename_managed_image
...
def test_prune_removes_unreferenced_keeps_referenced(managed_dirs, session):
    ...
    assert set(result["removed"]) == {str(orphan)}
