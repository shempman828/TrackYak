...
BasicStats: the statistics that existed before the library-statistics
expansion -- basic counts, play stats, ratings, metadata completeness,
audio quality, file formats, temporal breakdowns, and play/rating
leaderboards. Moved verbatim out of statistics_utility.py (which is now a
thin facade composing this and the other domain stat classes), with one
bug fix: get_ratings_distribution() now reports total_tracks from an
actual COUNT instead of rated+unrated (which silently dropped tracks whose
non-null rating fell outside the valid 0.5-10 range), and surfaces that
count explicitly as total_invalid_rating instead of hiding it.
"""
...
class BasicStats:
    """Core library statistics and analytics (counts, plays, ratings,
    completeness, audio quality, file formats, temporal, leaderboards)."""
    ...
    def get_ratings_distribution(self, session=None):
        ...
        total_tracks is a real COUNT(*), and any track whose user_rating is
        non-null but outside the valid 0.5-10 range is surfaced explicitly
        as total_invalid_rating rather than silently vanishing from both
        the rated and unrated buckets (the previous total_rated +
        total_unrated summation dropped those rows from the total
        entirely, which is what made the unrated count look wrong).
        """
        owns_session = session is None
        if owns_session:
            session = self.session_factory()
