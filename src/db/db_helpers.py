...
# Iterate over everything in src.db.db_tables
    ...
    def get_album_exists(self, album_name, release_year, artist_ids):
        ...
        try:
            ...
            for album_id, album_name in candidate_albums:
                ...
                from src.db.db_tables import AlbumRoleAssociation
...
_MERGE_MODEL_REGISTRY: dict = {
    "Artist": src.db.db_tables.Artist,
    "Publisher": src.db.db_tables.Publisher,
    "Genre": src.db.db_tables.Genre,
    "Mood": src.db.db_tables.Mood,
    "Role": src.db.db_tables.Role,
    "Album": src.db.db_tables.Album,
}
...
class SplitDB(BaseDBHelper):
    ...
    def split_publisher(self, publisher_id: int, new_names: list):
        ...
        original_publisher = self.session.get(src.db.db_tables.Publisher, publisher_id)
        ...
        try:
            ...
            for name in new_names:
                existing = self.session.scalar(
                    select(src.db.db_tables.Publisher).where(
                        src.db.db_tables.Publisher.publisher_name == name
                    )
                )
                if existing:
                    logger.debug(
                    ...
                    new_pub = src.db.db_tables.Publisher(
                        publisher_name=name,
            ...
            for new_pub in new_publishers:
                ...
                for album_id in album_ids:
                    if album_id not in existing_album_ids:
                        self._safe_add(
                            src.db.db_tables.AlbumPublisher(
                                album_id=album_id,
    ...
    def split_artist(self, artist_id: int, new_names: list):
        ...
        original_artist = self.session.get(src.db.db_tables.Artist, artist_id)
        ...
        try:
            ...
            for name in new_names:
                existing = self.session.scalar(
                    select(src.db.db_tables.Artist).where(
                        src.db.db_tables.Artist.artist_name == name
                    )
                )
                if existing:
                    logger.debug(
                    ...
                    new_artist = src.db.db_tables.Artist(artist_name=name)
                ...
                for new_artist in new_artists:
                    ...
                    if combo not in existing_track_roles:
                        added = self._safe_add(
                            src.db.db_tables.TrackArtistRole(
            ...
            for album_role in original_artist.album_roles:
                for new_artist in new_artists:
                    ...
                    if combo not in existing_album_roles:
                        added = self._safe_add(
                            src.db.db_tables.AlbumRoleAssociation(
    ...
    def split_genre(self, genre_id: int, new_names: list):
        ...
        original_genre = self.session.get(src.db.db_tables.Genre, genre_id)
        ...
        try:
            ...
            for name in new_names:
                existing = (
                    self.session.query(src.db.db_tables.Genre)
                    .filter(src.db.db_tables.Genre.genre_name == name)
                    .first()
                )
                if existing and existing.genre_id != genre_id:
                    ...
                    new_genre = src.db.db_tables.Genre(
                        genre_name=name,
    ...
    def split_mood(self, mood_id: int, new_names: list):
        ...
        original_mood = self.session.get(src.db.db_tables.Mood, mood_id)
        ...
        try:
            ...
            for name in new_names:
                existing = (
                    self.session.query(src.db.db_tables.Mood)
                    .filter(src.db.db_tables.Mood.mood_name == name)
                    .first()
                )
                if existing and existing.mood_id != mood_id:
                    ...
                    new_mood = src.db.db_tables.Mood(
                        mood_name=name,
            ...
            for mood_track in original_mood.mood_tracks:
                for new_mood in new_moods:
                    ...
                    if combo not in existing_mood_tracks:
                        added = self._safe_add(
                            src.db.db_tables.MoodTrackAssociation(
    ...
    def split_role(self, role_id: int, new_names: list):
        ...
        original_role = self.session.get(src.db.db_tables.Role, role_id)
        ...
        try:
            ...
            for name in new_names:
                existing = (
                    self.session.query(src.db.db_tables.Role)
                    .filter(src.db.db_tables.Role.role_name == name)
                    .first()
                )
                if existing and existing.role_id != role_id:
                    ...
                    new_role = src.db.db_tables.Role(
                        role_name=name,
