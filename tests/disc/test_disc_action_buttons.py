"""Regression tests for the Disc Management view's Edit/Remove disc buttons.

Bug: "Edit disc" and "Remove disc" never became active. The buttons start
disabled and were only re-enabled inside DiscManagementView.refresh_view(),
...
Also covers the newly implemented DiscManagementView.edit_disc().
"""

from PySide6.QtWidgets import QDialog

from src.disc import disc_view
...
def test_buttons_enabled_on_open_when_album_has_discs(qapp):
    view = DiscManagementView(_StubAlbum(), _StubController(discs=[_StubDisc()]))
...
def test_buttons_disabled_on_open_when_album_has_no_discs(qapp):
    view = DiscManagementView(_StubAlbum(), _StubController(discs=[]))
...
def test_edit_disc_writes_title_via_controller(qapp, monkeypatch):
    ...
    view = DiscManagementView(_StubAlbum(), controller)
