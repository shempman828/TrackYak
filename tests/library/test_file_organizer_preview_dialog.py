...
mood_word_review_widget.py's _WordTable: item heights here vary a lot (see
...
def test_ops_list_wheel_step_is_fixed_not_item_height_derived(qapp):
    ops = [
        {"current_path": "/music/a.mp3", "expected_path": "/music/sorted/a.mp3"},
        {
            ...
            "expected_path": "/music/sorted/" + ("very_long_path_segment_" * 10) + ".mp3",
        },
    ]
    ...
    try:
        item_heights = [
            dlg.ops_list.item(i).sizeHint().height() for i in range(dlg.ops_list.count())
        ]
        ...
        assert dlg.ops_list.verticalScrollMode() == QAbstractItemView.ScrollPerPixel
